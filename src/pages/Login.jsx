  ...
  const handleLogin = async (event) => {
    ...
    try {
      ...
      if (
        ...
        (error.message.includes("session") && error.code === 401)
      ) {
        setShowSessionConflict(true);
        setError(
          "Session conflict detected. Please clear your session and try again."
        );
      } else {
        ...
        <div className="w-full max-w-md backdrop-blur-sm bg-white/40 p-8 sm:p-10 lg:p-12 rounded-3xl shadow-2xl border border-white/30">
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-4 bg-white/40 backdrop-blur-sm text-gray-500 rounded-xl">
                or
              </span>
            </div>
          </div>
          ...
          <div className="mt-4 p-3 bg-blue-50/80 backdrop-blur-sm border border-blue-200/50 text-blue-700 rounded-xl">
            <p className="text-sm text-center">
              <span className="font-medium">Guest Mode:</span> Try DineFit
